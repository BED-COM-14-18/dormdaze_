import React, { useState} from 'react';
import { Link } from 'react-router-dom';
import { Sidebar } from 'primereact/sidebar';
import { Button } from 'primereact/button';
import { useNavigate } from 'react-router-dom';
import { BottomBar } from '../UserDashboardMobileComponent';



function MenuBt() {
    // Your existing MenuBt component
    
    const [visible, setVisible] = useState(false);
  
    return (
        <div className='flex justify-center'>
            <Sidebar visible={visible} onHide={() => setVisible(false)} className='flex flex-col bg-[#d5dce1eb] h-full shadow-md drop-shadow-md p-4 sm:w-[50] lg:w-[50vw] justify-center'>
              <div className='flex flex-col h-full justify-start'>
              <h2 className='text-2xl'>DormDash</h2>
                <ul className='flex flex-col justify-center h-64 gap-4'>
                  <Link to={'/adminDashboard'}>
                    <li className='flex flex-row gap-2 items-center sm:text-lg'>
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-shield-lock" viewBox="0 0 16 16">
                        <path d="M5.338 1.59a61 61 0 0 0-2.837.856.48.48 0 0 0-.328.39c-.554 4.157.726 7.19 2.253 9.188a10.7 10.7 0 0 0 2.287 2.233c.346.244.652.42.893.533q.18.085.293.118a1 1 0 0 0 .101.025 1 1 0 0 0 .1-.025q.114-.034.294-.118c.24-.113.547-.29.893-.533a10.7 10.7 0 0 0 2.287-2.233c1.527-1.997 2.807-5.031 2.253-9.188a.48.48 0 0 0-.328-.39c-.651-.213-1.75-.56-2.837-.855C9.552 1.29 8.531 1.067 8 1.067c-.53 0-1.552.223-2.662.524zM5.072.56C6.157.265 7.31 0 8 0s1.843.265 2.928.56c1.11.3 2.229.655 2.887.87a1.54 1.54 0 0 1 1.044 1.262c.596 4.477-.787 7.795-2.465 9.99a11.8 11.8 0 0 1-2.517 2.453 7 7 0 0 1-1.048.625c-.28.132-.581.24-.829.24s-.548-.108-.829-.24a7 7 0 0 1-1.048-.625 11.8 11.8 0 0 1-2.517-2.453C1.928 10.487.545 7.169 1.141 2.692A1.54 1.54 0 0 1 2.185 1.43 63 63 0 0 1 5.072.56"/>
                        <path d="M9.5 6.5a1.5 1.5 0 0 1-1 1.415l.385 1.99a.5.5 0 0 1-.491.595h-.788a.5.5 0 0 1-.49-.595l.384-1.99a1.5 1.5 0 1 1 2-1.415"/>
                      </svg>
                      admin dashboard</li>
                  </Link>
                  
                  <Link to={'/about/policies'}>
                    <li className='flex flex-row gap-2 items-center sm:text-lg'>
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-earmark-text" viewBox="0 0 16 16">
                        <path d="M5.5 7a.5.5 0 0 0 0 1h5a.5.5 0 0 0 0-1zM5 9.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5m0 2a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5"/>
                        <path d="M9.5 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.5zm0 1v2A1.5 1.5 0 0 0 11 4.5h2V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1z"/>
                      </svg>
                      terms and policies
                    </li>
                  </Link>
  
                  <li className='flex flex-row gap-2 items-center sm:text-lg'>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-info-square" viewBox="0 0 16 16">
                      <path d="M14 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/>
                      <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0"/>
                    </svg>
                    help
                  </li>
                  
                  <Link to={'/about/about'}>
                    <li className='flex flex-row gap-2 items-center sm:text-lg'>
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-sunglasses" viewBox="0 0 16 16">
                        <path d="M3 5a2 2 0 0 0-2 2v.5H.5a.5.5 0 0 0 0 1H1V9a2 2 0 0 0 2 2h1a3 3 0 0 0 3-3 1 1 0 1 1 2 0 3 3 0 0 0 3 3h1a2 2 0 0 0 2-2v-.5h.5a.5.5 0 0 0 0-1H15V7a2 2 0 0 0-2-2h-2a2 2 0 0 0-1.888 1.338A2 2 0 0 0 8 6a2 2 0 0 0-1.112.338A2 2 0 0 0 5 5zm0 1h.941c.264 0 .348.356.112.474l-.457.228a2 2 0 0 0-.894.894l-.228.457C2.356 8.289 2 8.205 2 7.94V7a1 1 0 0 1 1-1"/>
                      </svg>
                      about us
                    </li>
                  </Link>
                </ul>
              </div>
            </Sidebar>
            <Button icon="pi pi-arrow-right" onClick={() => setVisible(true)}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-list" viewBox="0 0 16 16" className='size-7'>
                <path fill-rule="evenodd" d="M2.5 12a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5m0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5m0-4a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5"/>
                </svg>
            </Button>
        </div>
    )
  }

  // function BottomBar() {
  //   return(
  //       <div className='bg-[#c8cedb] h-[6vh] w-full gap-5 flex flex-row justify-evenly items-center text-black fixed bottom-0 font-[calc(10px + 2vmin)]'>
  //           <Link to={'/hostelOwnerDashboard'}>
  //               <div className='flex flex-col items-center gap-0 text-black font-sans font-thin text-sm mt-[1px]'>
  //                   <button type="button" className='h-6 w-6 bg-cover bg-no-repeat' id='homeBt'></button>
  //                   <p>Home</p>
  //               </div>
  //           </Link>

  //           <Link to={''}>
  //           <div className='flex flex-col items-center gap-0 text-black font-sans font-thin text-sm mt-[1px]'>
  //               <button type="button" className='h-6 w-6 bg-cover bg-no-repeat'>
  //                   <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-bell" viewBox="0 0 16 16" className='size-5'>
  //                       <path d="M8 16a2 2 0 0 0 2-2H6a2 2 0 0 0 2 2M8 1.918l-.797.161A4 4 0 0 0 4 6c0 .628-.134 2.197-.459 3.742-.16.767-.376 1.566-.663 2.258h10.244c-.287-.692-.502-1.49-.663-2.258C12.134 8.197 12 6.628 12 6a4 4 0 0 0-3.203-3.92zM14.22 12c.223.447.481.801.78 1H1c.299-.199.557-.553.78-1C2.68 10.2 3 6.88 3 6c0-2.42 1.72-4.44 4.005-4.901a1 1 0 1 1 1.99 0A5 5 0 0 1 13 6c0 .88.32 4.2 1.22 6"/>
  //                   </svg>
  //               </button>
  //               <p>Inbox</p>
  //           </div>
  //           </Link>

  //           <Link to={'/createListings'}>
  //           <div className='flex flex-col items-center gap-y-1 justify-center text-black font-sans font-thin text-sm my-auto'>
  //               <button type="button" className='h-6 w-6 bg-cover bg-no-repeat'>
  //                 <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-square" viewBox="0 0 16 16" className='size-7 fill-black font-semibold'>
  //                   <path d="M14 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/>
  //                   <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4"/>
  //                 </svg>
  //               </button>
  //               <p>Create</p>
  //           </div>
  //           </Link>

  //           <Link to={'/HostelOwnerProfile'}>
  //               <div className='flex flex-col items-center gap-0 text-black font-sans font-thin text-sm mt-[1px]'>
  //                   <button type="button" className='h-6 w-6 bg-cover bg-no-repeat'>
  //                       <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person" viewBox="0 0 16 16" className='size-6'>
  //                           <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4m-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10s-3.516.68-4.168 1.332c-.678.678-.83 1.418-.832 1.664z"/>
  //                       </svg>
  //                   </button>
  //                   <p>Account</p>
  //               </div>
  //           </Link>
            
  //           <div className='flex flex-col items-center gap-y-0 text-black font-sans font-thin text-sm'>
  //               <MenuBt></MenuBt>
  //               <p>Menu</p>
  //           </div>
            
  //       </div>
  //   )
  // }


export default function HostelOwnerProfileMobileHomePage() {
    return(
        <>
        <div className='h-screen w-full flex flex-col items-center justify-between'>
            {/* <TopBar /> */}
            <BottomBar />
        </div>
        </>
    );
}